  ...
  <head>
	...
	<c:choose>
		<c:when test="${not empty param.country}">
			...
			<c:choose>
				<c:when test="${param.country == 'palau'}">
					<script type="text/javascript" src="layers.json?palau"></script>
				</c:when>
		      	<c:when test="${param.country == 'fiji'}">
					<script type="text/javascript" src="layers.json?fiji"></script>
				</c:when>
				<c:otherwise>
